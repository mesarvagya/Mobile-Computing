...
function scene:create( event )
    ...
    print("created lvl2 scene")
    -- Code here runs when the scene is first created but has not yet appeared on screen
    alex_kid_lvl2 = nil 
    boss_level2 = nil 
    alex_bubble_lvl2 = nil
    hand = nil
    created_lvl2 = true
    current_hand = 1
    match_count = 3
    ...
    boss_move_table = {[1] = "enemy2_rock", [2] ="enemy2_paper", [3] = "enemy2_scissor"};
    alex_bubble_lvl2_table = {[1] = "bubble_rock", [2] ="bubble_paper", [3] = "bubble_scissor"};

    --Loading the background
    local bg = display.newImage (background_sheet, 2);
        bg.x = display.contentWidth / 2;
        bg.y= display.contentHeight / 2;
        bg.xScale = display.contentWidth / bg.width; 
        sceneGroup:insert(bg) 
    --Loading the alex kid image
        alex_kid_lvl2 = alex_sequence
        alex_kid_lvl2.x = display.contentCenterX-80; 
        alex_kid_lvl2.y = display.contentCenterY+63; 
        alex_kid_lvl2.anchorX = 0; 
        alex_kid_lvl2.anchorY = 1;
        alex_kid_lvl2.alpha = 1
        sceneGroup:insert(alex_kid_lvl2)
    --Loading the bubble to show select move options
        alex_bubble_lvl2 = bubble_alex_sequence
        alex_bubble_lvl2.alpha = 1
        alex_bubble_lvl2.x = display.contentCenterX-90; 
        alex_bubble_lvl2.y = display.contentCenterY+15; 
        alex_bubble_lvl2.anchorX = 0; 
        alex_bubble_lvl2.anchorY = 1; 
        alex_bubble_lvl2.xScale = 1.2
        alex_bubble_lvl2.yScale = 1.2
        sceneGroup:insert(alex_bubble_lvl2);

    --Loading the boss/enemy here
        boss_level2 = janken_sequence
        boss_level2:setSequence("enemy2_set")
        boss_level2:setFrame(13)
        boss_level2.alpha = 1
        boss_level2.x = display.contentCenterX+80;
        boss_level2.y = display.contentCenterY+63;
        boss_level2.anchorX = 1;
        boss_level2.anchorY = 1;
        sceneGroup:insert( boss_level2)
...
function scene:show( event )
    ...
    function shoot()
        ...
        -- body
            ...
            alex_bubble_lvl2:setSequence(alex_bubble_lvl2_table[alex_hand]);
            alex_kid_lvl2:setSequence(alex_move)

            -- boss_level2:setSequence(boss_move_table[computer_move])
            --Computer move sprite
            boss_level2:setSequence("enemy2_set");
            hand = display.newImage (jankenSheet, 15 + computer_move, -- boss_rock
                 display.contentCenterX+58, 
                 display.contentCenterY+48
             );
            ...
            if(match_count > 0) then
                ...
                function ()
                    ...
                    selected_hand = nil
                    win_msg.text = ""
                    -- alex_bubble_lvl2.isEnabled = false
                    scene:show(event) 
                end,1)
            else
    ...
    if ( phase == "will" ) then
        ...
        if(not created_lvl2) then
            print("Lvl2 Okay scene again created")
            scene:create(event)
        end
        print("lvl2 scene already exits")
    ...
    elseif ( phase == "did" ) then
        ...
        function() -- This timer performs bubble change
            ...
            if (current_hand == 1) then
                alex_bubble_lvl2:setSequence("bubble_rock");
            elseif (current_hand == 2) then
                alex_bubble_lvl2:setSequence("bubble_paper");
            elseif (current_hand == 3) then
                alex_bubble_lvl2:setSequence("bubble_scissor");
            end
        ...
        -- body
            ...
            if(count_down.text == "0") then
                ...
                -- alex_bubble_lvl2.isEnabled = true
                alex_bubble_lvl2:addEventListener("tap", bubble_tap_handler);
                alex_kid_lvl2:setSequence ("alex_shake");
                alex_kid_lvl2:play();  
                boss_level2:setSequence("enemy2_shake");
                boss_level2:play()
